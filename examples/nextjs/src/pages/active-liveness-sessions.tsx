...
export default function ActiveLivenessSessions({
  data,
  ...
  return (
    <Container>
      <form action="/api/al-session" method="POST">
        <button
          ...
          type="submit"
        >
          Perform Active Liveness via session
        </button>
      </form>
    </Container>
  );
...
export const getServerSideProps: GetServerSideProps<{ data: Data }> = async ({
  ...
  const sid = query.sid;
  const data: Data = await vision.faceBio.activeLivenessSessions
    .retrieve({
      sid: sid as string,
    })
